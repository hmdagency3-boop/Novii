...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  // Refresh content only on double-click
  const handleNavDoubleClick = (path: string) => {
    ...
    >
      ...
      >
        ...
        <nav className="flex-1 space-y-2">
          ...
          />
          <NavItem href="/search" icon={Search} label={t.search} isActive={location === "/search"} onDoubleClick={() => handleNavDoubleClick('/search')} />
          <NavItem href="/explore" icon={Compass} label={t.explore} isActive={location === "/explore"} onDoubleClick={() => handleNavDoubleClick('/explore')} />
      ...
      <nav className={cn(
        ...
        <Link href="/">
          <button type="button" onDoubleClick={() => handleNavDoubleClick('/')} className="p-3 rounded-full active:scale-95 transition-transform cursor-pointer">
            <Home className={cn("w-6 h-6", location === "/" ? "text-primary stroke-[3px]" : "text-muted-foreground")} />
          </button>
        </Link>
        <Link href="/search">
          <div onDoubleClick={() => handleNavDoubleClick('/search')} className="p-3 rounded-full active:scale-95 transition-transform cursor-pointer">
            <Search className={cn("w-6 h-6", location === "/search" ? "text-primary stroke-[3px]" : "text-muted-foreground")} />
          </div>
        </Link>
        <Link href="/explore">
          <div onDoubleClick={() => handleNavDoubleClick('/explore')} className="p-3 rounded-full active:scale-95 transition-transform cursor-pointer">
            <Compass className={cn("w-6 h-6", location === "/explore" ? "text-primary stroke-[3px]" : "text-muted-foreground")} />
          </div>
        </Link>
        <Link href="/reels">
          <div onDoubleClick={() => handleNavDoubleClick('/reels')} className="p-3 rounded-full active:scale-95 transition-transform cursor-pointer">
