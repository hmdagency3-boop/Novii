...
export default function SuggestionsSidebar() {
  ...
  const getUserFollowingIds = (): Set<string> => {
    // Get following IDs from the fetched following users
  ...
  const SuggestionItem = ({ user }: { user: any }) => {
    const isFollowing = followingIds.has(user.id);

    return (
      <Link key={user.id} href={`/user?id=${user.id}`}>
        <div className="flex items-center justify-between p-3 hover:bg-accent/50 rounded-lg transition-colors group cursor-pointer">
          <div className="hover:opacity-80 transition-opacity flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="flex flex-col flex-1 min-w-0">
              ...
              <span className="text-xs text-muted-foreground truncate">
                {user.full_name || (isRTL ? 'مقترح لك' : 'Suggested for you')}
              </span>
            </div>
          </div>
          <button 
