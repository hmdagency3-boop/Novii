...
function NotificationRow({ notif, isAr, onRead }: { notif: any; isAr: boolean; onRead: () => void }) {
  ...
  const getHref = () => {
    ...
    if (notif.type === 'follow' || notif.type === 'follow_request') return actor?.username ? `/u/${actor.username}` : null;
    if (notif.post_id) {
  ...
  const rowContent = (
    ...
    >
      ...
      {isModeration ? (
        ...
        <Link href={`/u/${actor?.username}`} onClick={e => e.stopPropagation()} className="relative flex-shrink-0">
