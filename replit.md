...
- **Verification System**: Users submit verification requests from Settings → "التوثيق" tab. Admins review in admin panel "طلبات التوثيق" page. Server endpoints: `POST /api/verification/request` (submit), `GET /api/verification/status` (check own), `GET /api/admin/verification-requests` (list all with profile data), `PATCH /api/admin/verification-requests/:id` (approve/reject — pending only). Approval sets `is_verified: true` on profile. Backend validates reason, category enum, and sanitizes social_links (https only). Table: `verification_requests` (auto-created on startup).
