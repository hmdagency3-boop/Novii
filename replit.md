...
- **Moderation Notifications**: Admin actions (post removal, warnings, bans, unbans) send visible notifications to affected users. Reporter gets a "thank you" notification when their report is resolved. Notification types: `warning`, `post_removed`, `ban`, `unban`, `security`, `report_resolved`. Rendered in notifications page with distinct icons, amber highlight styling, and "Novii Admin" label. Clicking moderation notifications opens `/moderation/:id` detail page (Instagram-style) with full action details, community guidelines, and next steps.
