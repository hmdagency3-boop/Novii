-- ========================================
-- POST SETTINGS & FEATURES MIGRATIONS
-- ========================================
...
CREATE TABLE IF NOT EXISTS post_insights (
  ...
  reach INTEGER DEFAULT 0, -- عدد الأشخاص الذين رأوا البوست
  impressions INTEGER DEFAULT 0, -- عدد مرات ظهور البوست في الـ feed
...
-- ========================================
-- 6. CREATE TRIGGER - تحديث عداد المشاهدات تلقائياً
-- ========================================

-- إنشء function لتحديث عداد المشاهدات عند إدراج مشاهدة جديدة
...
-- ========================================
-- 8. VIEWS - طرق SQL للاستعلام السريع
-- ========================================

-- View لكل البوستات المثبتة
CREATE OR REPLACE VIEW all_pinned_posts AS
SELECT 
  p.*,
  pr.username,
  pr.avatar_url,
  pr.is_verified,
  (SELECT COUNT(*) FROM post_views WHERE post_id = p.id) as current_views,
  (SELECT COUNT(*) FROM saved_posts WHERE post_id = p.id) as current_saves
FROM posts p
...
-- ========================================
-- 9. HELPER FUNCTIONS - دوال مساعدة
-- ========================================
